...
public class A_IDS_A_15solver {
	...
	public  A_IDS_A_15solver(int[] puzzle, int type) {
		this.type = type;
		this.puzzle = puzzle;
	}
	...
	public ArrayList<Node> Solve() {
		...
		if (type == 1) {
			...
			return A_Star(startState, "heuristicOne");							//A* search with heuristic 1 (misplaced tiles)
		}
		else {
			System.out.println("\nStarting A* Search with heuristic #2....This may take a while\n\n");
			
			return A_Star(startState, "heuristicTwo");	
		}
 ...
 */
	public ArrayList<Node> A_Star(Node startState, String heuristic){
		...
		{
			ArrayList<Node> sol = start_A_Star.getSolutionPath(solution);	//creates ArrayList of solution path
			printSolution(sol);
			return sol;	//creates ArrayList of solution path
